	...
	CommandLineRunner initDatabase(EntrenamientoRepository repository) {
		return args -> {
			...
			if (repository.count() == 0) {
				repository.save(new Entrenamiento("Entrenamiento de Saque", "Práctica intensiva de saques largos", Categoria.TECNICA));
				repository.save(new Entrenamiento("Circuito de Fuerza", "Entrenamiento físico para resistencia y fuerza", Categoria.FISICO));
				repository.save(new Entrenamiento("Formaciones Estratégicas", "Formaciones avanzadas para competiciones", Categoria.ESTRATEGIA));
			} else {
